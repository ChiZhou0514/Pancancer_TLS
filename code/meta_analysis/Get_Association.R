...
Get_HR_continous_uni = function( status , time , score , age,sex,stage,data ){
  status <- as.numeric(status)
  time <- as.numeric(time)
  score <- as.numeric(score)
  age <- as.numeric(age)
  ...
  data = data.frame( status=status , time=time , score=score, age=age,sex=sex,stage=stage )
  data$time = as.numeric(as.character(data$time))
  data$variable = as.numeric( as.character(data$score) )
  
  cox = coxph( formula= Surv( time , status ) ~ score + age + sex + stage , data=data )
  mean <- round( coef( cox )[1] , 2 )
  low <- round( confint( cox , level=.95 )[ 1 , 1 ] , 2 )
  up <- round( confint( cox , level=.95 )[ 1 , 2 ] , 2 )
  pval <- summary(cox)$coef[1,5]
...
Get_HR_continous_ICB_uni = function( status , time , score , age,sex,data ){
  status <- as.numeric(status)
  time <- as.numeric(time)
  score <- as.numeric(score)
  age <- as.numeric(age)
  ...
  data = data.frame( status=status , time=time , score=score, age=age,sex=sex )
  data$time = as.numeric(as.character(data$time))
  data$variable = as.numeric( as.character(data$score) )
  
  cox = coxph( formula= Surv( time , status ) ~ score + age + sex , data=data )
  mean <- round( coef( cox )[1] , 2 )
  low <- round( confint( cox , level=.95 )[ 1 , 1 ] , 2 )
  up <- round( confint( cox , level=.95 )[ 1 , 2 ] , 2 )
  pval <- summary(cox)$coef[1,5]
...
Get_coef_continous_uni = function( score , response,age,sex ){
  score = as.numeric(score)
  response = as.numeric(response)
  age <- as.numeric(age)

  fit = glm( response ~ score + age + sex , family=binomial( link="logit" ) )
  coef <- round( summary(fit)$coefficients[ 2 ,  1  ] , 2 ) 
  se <- round( summary(fit)$coefficients[ 2 , 2 ] , 2 )
  low <- round( confint(fit)[ 2 , ] , 2 ) [ 1 ]
  up <- round( confint(fit)[ 2 , ] , 2 ) [ 2 ]
