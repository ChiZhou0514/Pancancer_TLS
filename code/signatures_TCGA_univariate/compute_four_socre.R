...
for (signatureID in c("IPS", "IPRES_score", "IFN_score", "COX_IS")){
    ...
    for (i in 1:length(names(TCGA_pheno))){
        ...
        pheno$score <- NA
        for (sample in intersect(pheno$sample, data_4_score$SAMPLE)){
            pheno[which(pheno$sample==sample), "score"] <- as.numeric(data_4_score[which(data_4_score$SAMPLE==sample), signatureID])
        }
