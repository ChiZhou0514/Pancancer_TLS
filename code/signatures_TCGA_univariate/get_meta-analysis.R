...
for (signatureID in signature_list){
    ...
    cox_os_hot = cox_os_hot[ cox_os_hot$SE > 0 & !is.na( cox_os_hot$Pval ) , ]
    #print(cox_os_hot)
    ...
    cox_os_cold = cox_os_cold[ cox_os_cold$SE > 0 & !is.na( cox_os_cold$Pval ) , ]
    ...
    cox_pfi_hot = cox_pfi_hot[ cox_pfi_hot$SE >= 0 & !is.na( cox_pfi_hot$Pval ) , ]
    ...
    cox_pfi_cold = cox_pfi_cold[ cox_pfi_cold$SE >= 0 & !is.na( cox_pfi_cold$Pval ) , ]
    if( nrow( cox_pfi[ !is.na( cox_os[ , 1 ] ) , ] ) ){
      # cancer <- Get_Cancer( cancer=cox_os )
      # seq <- Get_Seq( seq=cox_os )
