...
load("./TLS_project/data/TCGA_TMB.Rdata")

create_directory("TMB")
...
for (i in 1:length(names(TCGA_pheno))){
    print(names(TCGA_pheno)[i])
    pheno <- TCGA_pheno[[names(TCGA_pheno)[i]]]
    TMB_data <- TCGA_TMB[[names(TCGA_pheno)[i]]]
    
        
    pheno$score <- as.numeric(TMB_data[match(pheno$sample, TMB_data$sample_id), "TMB"])
    pheno <- pheno[which(!is.na(pheno$score)),]
    
    OS_KMplot_dir <- paste0("./TLS_project/results/signatures_TCGA_multivariate/", "TMB", "/KMPlot/OS/", names(TCGA_pheno)[i], ".pdf")
    PFI_KMplot_dir <- paste0("./TLS_project/results/signatures_TCGA_multivariate/", "TMB", "/KMPlot/PFI/", names(TCGA_pheno)[i], ".pdf")
...
save(cox_os, file = paste0("./TLS_project/results/signatures_TCGA_multivariate/", "TMB", "/COX_OS.Rdata"))
save(cox_pfi, file = paste0("./TLS_project/results/signatures_TCGA_multivariate/", "TMB", "/COX_PFI.Rdata"))
