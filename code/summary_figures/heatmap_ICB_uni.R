...
all_signatures <- c("gs_12_Chemokine", "gs_Clubb_James_H_A_etal", "gs_Meylan_Maxime_etal", "gs_PagliaruloFabio_etal", "gs_Wang_Bin_etal", "gs_Wu_Zhenghao_etal", "infiltration", "IPS", "IPRES_score", "IFN_score", "COX_IS")
res <- NULL
for( i in 1:length(all_signatures)){
  file = paste( "./TLS_project/results/signatures_ICB_uni/" , all_signatures[ i ] , "/Overall/Response/", all_signatures[ i ], "_Log_Response.RData" , sep="" )
  if( file.exists( file ) ){
    load( file )
...
res = NULL
for( i in 1:length(all_signatures)){
  file = paste( "./TLS_project/results/signatures_ICB_uni/" , all_signatures[ i ] , "/Overall/OS/", all_signatures[ i ], "_COX_OS.RData" , sep="" )
  if( file.exists( file ) ){
    load( file )
    res = rbind( res , c( meta_res , "sig" ) )
  }
...
pheatmap( t( data[ order( rowSums(data)) , ] ) , cluster_rows=FALSE , cluster_cols=FALSE , scale="none" , annotation_col = annotation_col, annotation_colors = ann_colors, 
          ...
          filename="./TLS_project/results/summary_figures/heatmap/Heatmap_Summary_ICB_uni.pdf", height=2.8, width=9.5 )  
...
pheatmap( t( data[ order( rowSums(data)) , ] ) , cluster_rows=FALSE , cluster_cols=FALSE , annotation_col = annotation_col, annotation_colors = ann_colors, 
          col = col , breaks = c( neg , pos ) , na_col="white" , border_color="#424242", angle_col = 315,  fontfamily="serif" ,
          #display_numbers = data , number_color="black" , fontsize_number= 10 , 
          filename="./TLS_project/results/summary_figures/heatmap/Heatmap_Summary_legend_ICB_uni.pdf", height=4, width=9.5 )  
