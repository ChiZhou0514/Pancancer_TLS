...
for (i in 1:length(names(TCGA_pheno))){
    ...
    pheno$score <- as.numeric(TMB_data[match(pheno$sample_id, TMB_data$sample_id), "TMB"])
    pheno <- pheno[which(!is.na(pheno$score)),]
        
    OS_KMplot_dir <- paste0("./TLS_project/results/siganatures_TCGA/", "TMB", "/KMPlot/OS/", names(TCGA_pheno)[i], ".pdf")
    PFI_KMplot_dir <- paste0("./TLS_project/results/siganatures_TCGA/", "TMB", "/KMPlot/PFI/", names(TCGA_pheno)[i], ".pdf")
...
save(cox_os, file = paste0("./TLS_project/results/siganatures_TCGA/", "TMB", "/COX_OS.Rdata"))
save(cox_pfi, file = paste0("./TLS_project/results/siganatures_TCGA/", "TMB", "/COX_PFI.Rdata"))
